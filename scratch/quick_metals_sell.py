...
logger = logging.getLogger("QuickMetalsSell")
...
def execute_quick_sell():
    ...
    for symbol in metals:
        ...
        # Calculate SL/TP for SELL (SL above, TP below)
    ...
    print("\n=== Sync Sell Confirmation ===")
    for o in orders:
        print(f"{o['symbol']} | Vol: {o['volume']} | Bid: {o['price']:.3f} | SL: {o['sl']:.3f} | TP: {o['tp']:.3f}")
        
    for request in orders:
        logger.info(f"Sending SELL order for {request['symbol']}...")
        ...
        if result is None:
            ...
            logger.info(f"Successfully placed SELL order on {request['symbol']}! Ticket: {result.order}")
